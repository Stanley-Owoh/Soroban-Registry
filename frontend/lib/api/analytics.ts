...
function generateTopSearchTerms() {
  return SEARCH_TERMS.slice(0, 20)
    .map((term) => ({
      term,
      count: rnd(50, 2000),
      growth: rnd(-30, 80),
...
function generateEngagementFunnel() {
  ...
  return [
    { stage: "Visitors", users: visitors, percentage: 100 },
    {
      stage: "Searched",
      users: searchers,
      percentage: Math.round((searchers / visitors) * 100),
    },
    {
      stage: "Viewed Contract",
      users: viewers,
      percentage: Math.round((viewers / visitors) * 100),
    },
    {
      stage: "Interacted",
      users: interactors,
      percentage: Math.round((interactors / visitors) * 100),
    },
    {
      stage: "Deployed",
      users: deployers,
      percentage: Math.round((deployers / visitors) * 100),
    },
  ];
}
